...
def encode_command(args: argparse.Namespace) -> bool:
    ...
    if file_type == 'unknown':
        ...
        print("  Supported formats: PNG, JPEG (images), WAV (audio), TXT, MD (text)")
        return False
    ...
    if file_type == 'image':
        success = encode_image(args.input, args.output, args.message, args.file)
    elif file_type == 'audio':
        success = encode_audio(args.input, args.output, args.message, args.file)
    elif file_type == 'text':
        method = args.text_method if hasattr(args, 'text_method') else 'whitespace'
        success = encode_text(args.input, args.output, args.message, args.file, method)
...
Supported formats:
  ...
  Text:   TXT, MD (whitespace or zero-width character encoding)
    ...
    # Encode command
    encode_parser = subparsers.add_parser('encode', help='Encode message into media file')
    ...
    message_group.add_argument('-f', '--file',
                              help='File containing message to encode')

    encode_parser.add_argument('--text-method', choices=['whitespace', 'zero_width'],
                              default='whitespace',
                              help='Method for text steganography (default: whitespace)')

    # Decode command
    decode_parser = subparsers.add_parser('decode', help='Decode message from media file')
    ...
    decode_parser.add_argument('-o', '--output',
                              help='Output file to save decoded message')
    decode_parser.add_argument('--text-method', choices=['whitespace', 'zero_width', 'auto'],
                              default='auto',
                              help='Method for text steganography (default: auto)')

    # Capacity command
    capacity_parser = subparsers.add_parser('capacity', help='Check media file capacity')
